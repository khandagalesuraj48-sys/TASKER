    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-2xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="text-sm font-bold text-slate-900">Database & Backend Status</h3>
          </div>
          {isConfigured ? (
        ...
        <div className="text-xs text-slate-600 space-y-3">
          ...
          <div className="p-3.5 bg-slate-50 rounded-lg border border-slate-200 space-y-2 font-mono text-[11px]">
            <div className="flex items-center justify-between">
              <span className="text-slate-500">SQL Migration File:</span>
              <button
                onClick={copyMigrationPath}
                className="inline-flex items-center gap-1 text-blue-600 hover:underline font-sans text-xs"
              >
                <Copy className="w-3 h-3" />
                <span>Copy Path</span>
              </button>
            </div>
            <code className="text-slate-800 block bg-white p-2 rounded border border-slate-200">
          ...
          <p className="text-[11px] text-slate-500 leading-relaxed">
            ...
            triggers, indexes, and configure the <code className="bg-slate-100 px-1 py-0.5 rounded">task-attachments</code> storage bucket.
          </p>
        </div>
      </div>

      {/* Single-User Identity Information */}
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-2xs space-y-3">
        ...
        <p className="text-xs text-slate-500">
          Configured single-user actor name for status transitions, completion audit records, and note remarks.
        </p>
        <div className="flex items-center gap-3 pt-2">
          <div className="px-3.5 py-2 rounded-lg bg-slate-100 font-semibold text-slate-800 text-xs">
            {DEFAULT_USER_NAME}
          </div>
          <span className="text-[11px] text-slate-400">
            (Configured via <code className="bg-slate-100 px-1 py-0.5 rounded">VITE_DEFAULT_USER_NAME</code> in .env)
          </span>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-2xs space-y-6">
        ...
        {/* Backup Progress */}
        {isBackingUp && (
          <div className="space-y-2 p-4 rounded-lg bg-blue-50 border border-blue-200 text-xs">
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-2xs space-y-6">
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-4">
          ...
          {isRestoring && (
            <div className="space-y-2 p-4 rounded-lg bg-emerald-50 border border-emerald-200 text-xs">
