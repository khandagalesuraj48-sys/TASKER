  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-foreground tracking-tight">Organizations Management</h1>
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-10 pr-4 py-2.5 rounded-lg bg-card border border-border/80 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/20"
        />
      </div>
                ...
                <tr key={org.id} className="hover:bg-muted/40 transition-colors">
                  <td className="py-3.5">
                    <div className="flex items-center gap-2.5">
                      ...
                      <div>
                        <p className="font-semibold text-foreground text-xs">{org.legal_name}</p>
                  ...
                  <td className="py-3.5 text-muted-foreground">
                    <span className="px-2 py-0.5 rounded bg-muted border border-border/60 font-mono text-[10px] text-foreground">
                      {org.currency || 'INR'}
                    </span>
                  </td>
                  <td className="py-3.5">
                  ...
                  <td className="py-3.5 text-right space-x-2">
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-all border bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 border-emerald-500/20 inline-flex items-center gap-1 shadow-2xs"
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-all border bg-primary/10 hover:bg-primary/20 text-primary border-primary/20 inline-flex items-center gap-1 shadow-2xs"
                    >
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 rounded-lg text-[11px] font-semibold transition-all border bg-rose-500/10 hover:bg-rose-500/20 text-rose-600 dark:text-rose-400 border-rose-500/20 inline-flex items-center gap-1 shadow-2xs disabled:opacity-50"
            ...
            <form onSubmit={handleCreateOrg} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-lg bg-background border border-input/80 text-foreground text-xs focus:outline-none focus:ring-2 focus:ring-primary/20"
                />
              </div>
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="w-4 h-4 rounded text-primary bg-background border-border/80 focus:ring-primary/20"
                />
                <label htmlFor="org_active" className="text-xs text-foreground font-medium cursor-pointer">
              ...
              <div className="flex items-center justify-end gap-2 pt-3">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-xs font-semibold rounded-lg shadow-xs transition-all disabled:opacity-50"
                >
      ...
      {selectedOrgForSites && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-xs">
          <div className="bg-card border border-border rounded-2xl p-6 max-w-xl w-full shadow-2xl space-y-4 text-foreground">
            ...
            <div className="flex items-center justify-between">
              <span className="text-xs text-muted-foreground font-semibold">Active Project Sites</span>
              <button
                onClick={() => setShowAddSiteForm(!showAddSiteForm)}
                className="px-2.5 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-semibold transition-all flex items-center gap-1 shadow-2xs"
              >
            ...
            {showAddSiteForm && (
              <form onSubmit={handleCreateSite} className="p-3 bg-muted/40 border border-border/70 rounded-xl space-y-3">
                <p className="text-xs font-semibold text-foreground">Create New Work Site</p>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[11px] text-muted-foreground mb-1">Site Name *</label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full px-3 py-1.5 rounded-lg bg-background border border-input/80 text-foreground text-xs"
                    />
                  </div>
                  <div>
                    <label className="block text-[11px] text-muted-foreground mb-1">Site Code *</label>
                    <input
                      type="text"
                      required
                      ...
                      className="w-full px-3 py-1.5 rounded-lg bg-background border border-input/80 text-foreground text-xs font-mono uppercase"
                    />
                  </div>
                </div>
                <div className="flex justify-end gap-2">
                  <button
                    ...
                    className="px-3 py-1 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-semibold shadow-2xs disabled:opacity-50"
                  >
            ...
            {loadingSites ? (
              ...
              <div className="max-h-64 overflow-y-auto space-y-2 pr-1">
                {orgSites.map((s) => (
                  <div
                    key={s.id}
                    className="p-3 bg-muted/30 border border-border/60 rounded-xl flex items-center justify-between gap-3 text-xs"
                  >
                    <div className="flex items-center gap-2.5">
                      <MapPin className="w-4 h-4 text-emerald-500 shrink-0" />
                      <div>
                        <p className="font-semibold text-foreground">{s.name}</p>
                        <p className="text-muted-foreground text-[10px] font-mono">Code: {s.code}</p>
                      </div>
                    </div>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase bg-emerald-500/10 border border-emerald-500/30 text-emerald-600 dark:text-emerald-400">
                      Active
                    </span>
                  </div>
                ))}
            ...
            <div className="flex justify-end pt-2">
              <button
                type="button"
                onClick={() => setSelectedOrgForSites(null)}
                className="px-4 py-2 bg-secondary hover:bg-secondary/80 text-secondary-foreground text-xs font-semibold rounded-lg shadow-2xs transition-all"
              >
                Close
              </button>
            </div>
            ...
            {loadingMembers ? (
              ...
              <div className="max-h-72 overflow-y-auto space-y-2 pr-1">
                {orgMembers.map((m) => (
                  ...
                  >
                    <div>
                      <p className="font-mono text-foreground text-[11px]">{m.user_id}</p>
